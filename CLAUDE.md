...
The page supports a **static QA mode**: append `?qa=1` to render the final, non-animated state, force-eager all images, and cap the hero. Use `?qa=1&go=<sectionId>` is unreliable in headless; instead capture a tall full page (`--window-size=1440,12000`) and crop with `System.Drawing`, or capture per-section by viewport. `?qa=1&lb=<categoryId>` opens the lightbox for verifying it.

Working command:
```
$out = "$env:TEMP\annashots\name.png"   # space-free path
& "C:\Program Files\Google\Chrome\Application\chrome.exe" --headless=new --no-first-run `
  ...
  --window-size=1440,12000 --screenshot="$out" "http://localhost:8123/?qa=1"
```
